...
String test_info = "/home/capstone/pipeline/AdaBoost_to_CNN/faces_log.txt";

std::vector<Rect> overall(std::vector<Rect> faces);
std::vector<String> getImageNames (std::vector<double> &type);
...
int main( int argc, char** argv )
{
	...
	for (unsigned int num = 0; num < image.size(); num++){//per image in the folder	
		...
		Mat frame_gray;
		if(num==0){
			frame=imread(org_image,1);
		}
		else    frame = imread("../output.jpg", 1 );
		...
		if ( frame.data ){
			...
			Mat * temp = &frame;
			cout<<image[num]<<" "<<score[num]<<endl;
			if(score[num]!=0) draw_rects(temp, faces_ans,Scalar(0,0,255),5,0);
			//else draw_rects(temp, faces_ans, Scalar(0,0,0),10,0);
