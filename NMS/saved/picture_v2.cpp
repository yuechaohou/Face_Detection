...
String test_info = "new_positive_file_log.txt";
String neg_info = "new_negative_file_log.txt";

std::vector<Rect> overall(std::vector<Rect> faces);
std::vector<String> getImageNames (void);
std::vector<Rect> getAns(String image_name, int type);
...
int main( int argc, char** argv )
{
	...
	for (unsigned int num = 0; num < image.size(); num++){//per image in the folder	
		...
		if ( frame.data ){
			...
			std::vector<Rect> neg_faces;
			faces_ans = getAns(image[num],1);
			Mat * tmp = &frame;
			draw_rects(tmp, faces_ans, 255,3,0);
			neg_faces = getAns(image[num],0);
			Mat * temp = &frame;
			draw_rects(temp, neg_faces, 0,3,0);				
		}
...
std::vector<Rect> getAns (String image_name, int type){
	...
	ifstream filestream;
	string filename;
	if (type==1) filename= test_info;
	else filename = neg_info;
