...
string output_file_left="/home/capstone/pipeline/AdaBoost_to_CNN/faces_log_right.txt";
...
int main( int argc, char** argv )
{
	...
	//final frame size 1/10 
        right_face_cascade.detectMultiScale( frame_gray, right_faces, 1.1, 4, 0, Size(frame.size().height/10, frame.size().width/10) , frame.size() );  
...
void CutRect(std::string image_path, std::vector<Rect>& ResultFaces, Mat* ptr) {
    ...
    for( size_t i = 0; i < ResultFaces.size(); i++ )
    {
        ...
        if((ResultFaces[i].x > 0) && (ResultFaces[i].y > 0) && 
            ...
            std::string path="/home/capstone/pipeline/AdaBoost_to_CNN/detected_faces_right/";
