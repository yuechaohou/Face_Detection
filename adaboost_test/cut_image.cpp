...
string positive_file_directory="/home/capstone/adaboost/adaboost_test/new_positive_file_log_right.txt";
string negative_file_directory="/home/capstone/adaboost/adaboost_test/new_negative_file_log_right.txt";
...
void CutRect(std::string image_path, std::vector<Rect>& ResultFaces, Mat* ptr) {
    ...
    for( size_t i = 0; i < ResultFaces.size(); i++ )
    {
        ...
        if((ResultFaces[i].x > 0) && (ResultFaces[i].y > 0) && 
            ...
            std::string path;
            if (overlap_bool(ResultFaces[i], images_info[image_name], 80, 0)) {
                ...
                file_positive<<ResultFaces[i].x<<' '<<ResultFaces[i].y<<' '<<ResultFaces[i].width<<' '<<ResultFaces[i].y<<endl;
                //imwrite(path+name, croppedFaceImage);
                positive_count++;
            } else if (!overlap_bool(ResultFaces[i], images_info[image_name], 20, 0)) {
                path = "/home/capstone/adaboost/adaboost_test/new_negative/";
                std::string name = image_name + "_" + to_string(i) + ".jpg";
                // cout << "writing to " << path << name << endl;
                file_negative<<image_path<<endl;
                file_negative<<ResultFaces[i].x<<' '<<ResultFaces[i].y<<' '<<ResultFaces[i].width<<' '<<ResultFaces[i].y<<endl;
                //imwrite(path+name, croppedFaceImage);
                negative_count++;
            }
        }
